...
func Ex49() {
	nameAndFavoriteThings := make(map[string][]string)
	nameAndFavoriteThings["bond_james"] = []string{"shaken", "not stirred", "martinis", "fast cars"}
	nameAndFavoriteThings["moneypenny_jenny"] = []string{"intelligence", "literature", "computer science"}
	nameAndFavoriteThings["no_dr"] = []string{"cats", "ice cream", "sunsets"}

	PrintMap(nameAndFavoriteThings)

	fmt.Println("adding 006")
	nameAndFavoriteThings["006"] = []string{"satellite dishes", "betrayal", "dying"}

	PrintMap(nameAndFavoriteThings)

	fmt.Println("deleting moneypenny")
	delete(nameAndFavoriteThings, "moneypenny_jenny")
...
func PrintMap(m map[string][]string) {
	for k, v := range m {
		...
		for i, sliceVal := range v {
			fmt.Println("index:", i, " value:", sliceVal)
		}
	}
}
